...
import com.airbnb.clone.server.listing.application.LandlordService;
...
import com.airbnb.clone.server.user.application.UserException;
...
/**
 * @Created 5/31/2024 - 4:06 PM on (Friday)
 * @Package com.airbnb.clone.server.listing.presentation
...
public class LandlordResource {
    ...
    public ResponseEntity<CreatedListingDTO> create(MultipartHttpServletRequest request, @RequestPart(name = "dto") String saveListingDTOString) throws IOException {
        ...
        if (!violations.isEmpty()) {
            String violationsJoined = violations.stream().map(violation -> violation.getPropertyPath() + " " + violation.getMessage()).collect(Collectors.joining());
